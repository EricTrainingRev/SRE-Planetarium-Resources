...
public class MoonController {
    ...
    public ResponseEntity<String> primaryKeyViolation(PSQLException e){
        moonLogger.error(e.getLocalizedMessage(), e);
    ...
    public ResponseEntity<Moon> createMoon(@RequestBody Moon newMoon) {
    ...
    public ResponseEntity<String> deleteMoon(@PathVariable String name) {
        if (this.moonService.deleteMoon(name)){
            return new ResponseEntity<>("moon deleted", HttpStatus.OK);
        } else {
            throw new EntityNotFound("Moon could not be deleted");
        }
    }
